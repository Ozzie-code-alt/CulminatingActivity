...
import IloiloSpline from './SplineComponent';
import SubVideoSection from './SubSectionVideo';
...
const Section1 = ({ scrollYProgress }) => {
  ...
  return (
    <>
      <motion.section
        style={{ scale, rotate }}
        className='sticky  font-semibold top-0 h-screen bg-gradient-to-t to-[#dadada] from-[#ebebeb]  hidden md:flex flex-col items-center justify-center text-black'
      >
        <div className='absolute bottom-0 left-0 right-0 top-0 bg-[linear-gradient(to_right,#4f4f4f2e_1px,transparent_1px),linear-gradient(to_bottom,#4f4f4f2e_1px,transparent_1px)] bg-[size:54px_54px] [mask-image:radial-gradient(ellipse_90%_70%_at_50%_0%,#000_70%,transparent_100%)]'>
          <IloiloSpline />
        </div>

        {/* <h1 className='2xl:text-7xl text-center text-6xl px-8 font-semibold  tracking-tight leading-[120%]'>
        IloIlo <br /> Super Mart 
        </h1> */}
      </motion.section>

      <motion.section
        style={{ scale, rotate }}
        className='sticky  font-semibold  h-screen flex md:hidden flex-col items-center justify-center text-black'
      >
        <SubVideoSection />
